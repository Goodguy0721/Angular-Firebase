...
app.controller('main', function($scope, $firebaseArray, $firebaseObject, $firebaseAuth, $state, $timeout, $rootScope, $http, Pagination) {
	...
	$scope.item_total_Weight = 0;
	$scope.item_total_material_cost = 0;
	$scope.item_total_labor_cost = 0;

	$scope.showItemGroup = {};
	$scope.viewingPDF = false;
	/* start Pagination set*/
	...
	$scope.$on('$viewContentLoaded', function(event){ 
		...
		if($state.current.name == 'edit.estimateID'){
			...
			$scope.itemGroups.$loaded().then(function(){
				var item_total_Weight = 0;
				var item_total_material_cost = 0;
				var item_total_labor_cost = 0;

		    	item_length = $scope.itemGroups.length;
		    	...
		    	for(var i = 0 ; i < $scope.itemGroups.length ; i++) {
					item_total_Weight += parseFloat($scope.itemGroups[i]['weight']);
					item_total_material_cost += parseFloat($scope.itemGroups[i]['materialCost']);
					item_total_labor_cost += parseFloat($scope.itemGroups[i]['laborCost']);
				}
			
				$scope.item_total_Weight = item_total_Weight;
				$scope.item_total_material_cost = item_total_material_cost;
				$scope.item_total_labor_cost = item_total_labor_cost;
	...
	$scope.removeItems = function(itemGroup) {
		...
		item_length --;
		if (item_length % $scope.pagination.perPage == 0)
		{
	...
	$scope.addLapBars = function(lap) {
		...
		if(lap) {
			...
			lap.secondmultiplier = lap.savedSecondMultiplier;
		}

		item_length ++;//yuri
		if (item_length % $scope.pagination.perPage == 1)
		{
			$scope.pagination.numPages ++;
